...
Class UpLoadClass
	...
	Public Function Open()
		...
		rs8.open "select * from tbl_user where userId="&request("userId")&" order by userId desc",conn,3,3
		Open = 0
		if m_Error=-1 then
			m_Error=0
		else
		...
		' 修改用户头像文件					
							rs8("userImg") = "uploadPic\plupload\" + strFnam
							rs8.update
							rs8.close
...
uploadpath =  "plupload/" '路径
uploadsize="1024" '大小
uploadtype="jpg/gif/png/bmp/txt/exe/mp4" '后缀
Set Uprequest=new UpLoadClass
  ...
  response.Write "{""jsonrpc"":""2.0"",""result"":""失败"",""id"":""id""}"
  
  else
  response.Write "{""jsonrpc"":""2.0"",""result"":""成功"",""id"":""id""}"
  end if
