...
rs8.Open "select * from tbl_user where userId<>"&session("userId")&"",conn,3,3
...
' 提交	
	set rs = server.CreateObject("ADODB.RecordSet")
	rs.Open "select * from tbl_user where userId="&upload.form("userId")&"",conn,3,3	
	if FileNameOK <> "" then
	rs("userImg") = "/adm/img/"&FileNameOK 
	end if 
...
<form method="post"  name="addForm"  onSubmit="return CheckPost()" enctype="multipart/form-data" action="admProfileSave.html">
